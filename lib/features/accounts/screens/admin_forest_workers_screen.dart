...
class AdminForestWorkersScreen extends StatefulWidget {
  ...
  @override
  State<AdminForestWorkersScreen> createState() => _AdminForestWorkersScreenState();
}
...
class _AdminForestWorkersScreenState extends State<AdminForestWorkersScreen> {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          ...
          'Lưu ý: thao tác này chỉ xóa dữ liệu Firestore. Tài khoản Auth cần xóa bằng Firebase Console.',
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                await _db.deleteUserProfile(worker.uid);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Đã xóa hồ sơ "${worker.fullName}"')),
                  );
                }
              } catch (e) {
                if (mounted) {
  ...
  Future<void> _toggleStatus(UserModel worker) async {
    final next = worker.status == UserStatus.active ? UserStatus.locked : UserStatus.active;
    try {
  ...
  void _showWorkerDialog({UserModel? worker, required UserModel owner}) {
    ...
    final forestAreaController = TextEditingController(
      text: worker?.forestName.isNotEmpty == true ? worker!.forestName : owner.forestName,
    );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      StreamBuilder<List<ForestProjectModel>>(
                        stream: ForestProjectService.instance.getProjectsByOwnerStream(owner.uid),
                ...
                onPressed: isSubmitting ? null : () async {
                  ...
                  if (createAccount && gmailController.text.trim().isNotEmpty) {
                    ...
                    try {
                      ...
                      if (!context.mounted) return;
                      Navigator.pop(context);
                      _showSuccessSnack('Đã tạo worker cho "${userModel.fullName}"');
                    } catch (e) {
                      setDialogState(() => isSubmitting = false);
                      _showErrorSnack('Lỗi tạo worker: $e');
                    }
            ...
            child: StreamBuilder<List<UserModel>>(
              ...
              builder: (context, snapshot) {
                ...
                if (_filterStatus != null) {
                  workers = workers.where((w) => w.status == _filterStatus).toList();
                }

                if (_filterOwnerId != null && _filterOwnerId!.isNotEmpty) {
                  workers = workers.where((w) => w.ownerId == _filterOwnerId).toList();
                }
                ...
                final allWorkers = (snapshot.data ?? [])
                    .where((u) => u.role == UserRole.forestWorker)
                    .toList();
                final activeCount = allWorkers.where((w) => w.status == UserStatus.active).length;
                final lockedCount = allWorkers.where((w) => w.status == UserStatus.locked).length;

                // Get unique owners
                final owners = (snapshot.data ?? [])
                    .where((u) => u.role == UserRole.forestOwner)
                    .toList();
                ...
                return Column(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        ElevatedButton.icon(
                          onPressed: owners.isEmpty
                              ? null
                              : () => _showWorkerDialog(worker: null, owner: owners.first),
                    ...
                    if (workers.isEmpty)
                      const AppEmptyState(title: 'Chưa có Forest Worker nào hoặc không tìm thấy.')
                    else
            ...
            child: DropdownButton<String?>(
              ...
              items: [
                DropdownMenuItem<String?>(
                  value: null,
                  child: Text('Tất cả chủ rừng', style: GoogleFonts.inter(fontSize: 13)),
                ),
                ...owners.map((owner) => DropdownMenuItem<String?>(
                  value: owner.uid,
                  child: Text(owner.fullName, style: GoogleFonts.inter(fontSize: 13)),
                )),
              ],
            ...
            child: DropdownButton<UserStatus?>(
              ...
              items: [
                ...
                DropdownMenuItem<UserStatus?>(
                  ...
                  child: Text('Hoạt động', style: GoogleFonts.inter(fontSize: 13)),
                ),
                DropdownMenuItem<UserStatus?>(
                  value: UserStatus.locked,
                  child: Text('Đã khóa', style: GoogleFonts.inter(fontSize: 13)),
                ),
              ],
        ...
        child: ConstrainedBox(
          ...
          child: Theme(
            ...
            child: DataTable(
              ...
              columns: const [
                DataColumn(label: Padding(padding: EdgeInsets.only(left: 16), child: Text('Họ tên'))),
              ...
              rows: workers.map((worker) {
                ...
                return DataRow(
                  cells: [
                    ...
                    DataCell(Text(worker.email.isEmpty ? '—' : worker.email,
                        style: GoogleFonts.inter(color: AppColors.secondary, fontSize: 12))),
                    DataCell(Text(worker.ownerName.isEmpty ? '—' : worker.ownerName)),
                    DataCell(Text(worker.forestName.isEmpty ? '—' : worker.forestName)),
                      ...
                      Container(
                        ...
                        child: Text(
                          ...
                          style: GoogleFonts.inter(
                            ...
                            color: isActive ? AppColors.tertiary : Colors.orange,
                          ),
                        ),
                      ),
                    ),
                      ...
                      Row(
                        ...
                        children: [
                          IconButton(
                            ...
                            tooltip: 'Chỉnh sửa',
                            onPressed: () => _showWorkerDialog(worker: worker, owner: owner),
                          ),
                          IconButton(
                            icon: Icon(
                              isActive ? Icons.lock_outline : Icons.lock_open,
                              size: 20,
                              color: isActive ? Colors.orange : AppColors.tertiary,
                            ),
